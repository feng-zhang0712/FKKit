...
iOS 组件库：`FKButton`、`FKBar`、`FKPresentation`、`FKBarPresentation` 及共享模块 `FKUIKitCore`。
...
| FKBarPresentation | `FKBarPresentation` | 依赖 `FKBar`、`FKPresentation`、`FKUIKitCore` |
...
let presentation = FKBarPresentation(frame: .zero)
presentation.reloadBarItems([/* FKBar.Item */])
```
