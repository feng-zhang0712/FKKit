...
final class FKFilterSingleListPanelExampleDetailViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    let panel = FKFilterSingleListViewController(
      ...
      onChange: { [weak self] _ in self?.appendLog("onChange(FKFilterSection)") },
      onSelection: spec.deliversSelectionEvents
        ? { [weak self] sel in
            self?.appendLog(
              "onSelection sectionID=\(sel.sectionID?.rawValue ?? "nil") item=\(sel.item.id.rawValue) mode=\(String(describing: sel.effectiveSelectionMode))"
            )
          }
        : nil,
