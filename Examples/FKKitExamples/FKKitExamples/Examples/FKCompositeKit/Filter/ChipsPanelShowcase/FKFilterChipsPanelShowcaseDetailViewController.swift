  ...
  override func viewDidLoad() {
    ...
    let panel = FKFilterChipsViewController(
      ...
      onChange: { [weak self] updated in
        self?.appendLog("onChange([FKFilterSection]) count=\(updated.count)")
      },
      onSelection: spec.deliversSelectionEvents
        ? { [weak self] sel in
            self?.appendLog(
              "onSelection section=\(sel.sectionID?.rawValue ?? "nil") item=\(sel.item.id.rawValue) mode=\(String(describing: sel.effectiveSelectionMode))"
            )
          }
        : nil,
