...
/// Scrollable actions and monospace log shared by background task scenario demos.
@MainActor
class FKBackgroundTaskExampleBaseViewController: UIViewController {
  ...
  func addMonospaceLabel(_ text: String) {
    let label = UILabel()
  ...
  func runTask(_ label: String, operation: @escaping () async throws -> Void) {
    ...
    Task { @MainActor [weak self] in
      ...
      do {
        ...
        self.appendLog("[\(label)] error: \(FKBackgroundTaskExampleSupport.formatError(error))")
      }
    }
  }

  func refreshExecutionLog(prefix: String = "Handler execution store") {
    Task { @MainActor [weak self] in
      let lines = await FKBackgroundTaskExampleExecutionStore.shared.snapshot()
      self?.appendLog("\(prefix) (\(lines.count) entries):")
      if lines.isEmpty {
        self?.appendLog("  (empty — use Xcode → Debug → Simulate Background Tasks on device)")
      } else {
        lines.suffix(8).forEach { self?.appendLog("  \($0)") }
      }
    }
  }
