...
/// Demonstrates ``FKListSkeletonPolicy/presetRows(count:)`` placeholder cells on table initial load.
final class FKListKitSkeletonPresetRowsExampleViewController: FKDiffableTableViewController, FKListDataProviding {
  init() {
    var config = FKListDefaults.defaultConfiguration
    config.loading.usesSkeletonForInitialLoad = true
    config.loading.skeletonPolicy = .presetRows(count: 8)
  ...
  func fetchInitial(page: Int) async throws -> FKListFetchResult {
    let result = try await FKListKitExampleFeedAPI.fetch(page: page, delay: 2.0, itemsPerPage: 12)
