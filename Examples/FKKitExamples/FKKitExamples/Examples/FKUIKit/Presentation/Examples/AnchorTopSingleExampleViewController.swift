...
final class AnchorTopSingleExampleViewController: UIViewController {
  ...
  private func makeControls() -> UIView {
    ...
    subtitle.text = "Controls (live config)"
  ...
  private func presentPopup() {
    ...
    let embedded = FKEmbeddedAnchorConfig(
      anchor: anchor,
    ...
    var config = FKPresentationConfiguration()
    config.mode = .anchorEmbedded(embedded)
    config.backdrop.style = .dim(color: .black, alpha: CGFloat(maskAlpha))
    config.cornerRadius = CGFloat(cornerRadius)
    if !showsShadow {
      config.shadow.opacity = 0
      config.shadow.radius = 0
      config.shadow.offset = .zero
    }
