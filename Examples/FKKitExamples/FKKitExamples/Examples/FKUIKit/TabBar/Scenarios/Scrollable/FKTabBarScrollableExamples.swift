...
final class FKTabBarLongTitleStrategyExampleViewController: UIViewController {
  ...
  private lazy var tabView = FKTabBar(
    items: Array(FKTabBarExampleSupport.makeLongTitleItems().prefix(3)),
    selectedIndex: 1,
  ...
  override func viewDidLoad() {
    ...
    stack.addArrangedSubview(
      FKTabBarExampleSupport.captionLabel(
        "Truncate / Shrink / Auto use a non-scrollable equal-width strip so each tab has limited width. Fixed switches to scrollable fixed-width columns. Compare the selected middle tab."
      )
    )
  ...
  private func applyMode() {
    ...
    case 0:
      ...
      configuration.layout.nonScrollableOverflowPolicy = .truncate
    case 1:
      configuration.layout.isScrollable = false
      configuration.layout.widthMode = .fillEqually
      configuration.layout.titleOverflowMode = .shrink(minimumScaleFactor: 0.75)
      configuration.layout.nonScrollableOverflowPolicy = .shrink
    case 2:
      configuration.layout.isScrollable = false
      configuration.layout.widthMode = .fillEqually
    ...
    default:
      ...
      configuration.layout.titleOverflowMode = .fixedWidth(108)
    }
