...
///
/// Key highlights:
/// - `FKSheetPresentationLifecycleHandlers.progress` updates a label in real time.
/// - A common use case is to synchronize backdrop or chrome with dismiss progress.
final class InteractiveDismissProgressExampleViewController: FKSheetPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present") { [weak self] in
      ...
      let callbacks = FKSheetPresentationLifecycleHandlers(
      ...
      self.activePresentation = FKSheetPresentationExampleHelpers.present(
        from: self,
