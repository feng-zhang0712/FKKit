...
final class CustomAnimationTimingExampleViewController: FKSheetPresentationExamplePageViewController {
  ...
  override func viewDidLoad() {
    ...
    addPrimaryButton(title: "Present") { [weak self] in
      ...
      configuration.animation.duration = TimeInterval(self.duration)
      configuration.animation.dampingRatio = CGFloat(self.damping)
      configuration.animation.response = CGFloat(self.response)

      _ = FKSheetPresentationExampleHelpers.present(from: self, title: "Custom timing", configuration: configuration)
    }
  }
}
