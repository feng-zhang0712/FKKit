...
final class FKRefreshDefaultDemoViewController: UIViewController {
  ...
  private lazy var outcomeControl: UISegmentedControl = {
    ...
    s.addTarget(self, action: #selector(outcomeSegmentChanged), for: .valueChanged)
    return s
  }()
  ...
  @objc private func outcomeSegmentChanged() {
  ...
  private func runPullRefresh() {
    ...
    FKRefreshExampleCommon.simulateRequest(delay: delay) { [weak self] in
      ...
      default:
        self.page = 1
        self.items = (1...self.pageSize).map { "Row \($0)" }
        self.tableView.reloadData()
        self.tableView.fk_pullToRefresh?.endRefreshing()
        // `resetToIdle()` does not re-arm automatic load-more; use footer reset so pagination works after a prior bottom load (especially visible with the Fast delay).
  ...
  private func runLoadMore() {
    ...
    FKRefreshExampleCommon.simulateRequest(delay: delay) { [weak self] in
      ...
      self.page += 1
      self.tableView.reloadData()
      // Must not call `endRefreshing()` before `endRefreshingWithNoMoreData()`: the first call leaves
      // `.finished`, so the second guard (`state == .loadingMore`) fails and “no more” never applies.
      if self.items.count >= self.maxRows {
        self.tableView.fk_loadMore?.endRefreshingWithNoMoreData()
      } else {
