...
final class FKFileStorageCore: FKFileOperating, FKFileContentStoring {
  ...
  func writeContent(_ content: FKFileContent, to url: URL, atomically: Bool = true) async throws {
    do {
      ...
      case let .text(text, encoding):
        guard let data = text.data(using: encoding) else { throw FKFileManagerError.unknown(FKI18n.string("fkcore.file.error.text_encoding_failed")) }
  ...
  func readText(from url: URL, encoding: String.Encoding = .utf8) async throws -> String {
    ...
    guard let text = String(data: data, encoding: encoding) else { throw FKFileManagerError.unknown(FKI18n.string("fkcore.file.error.text_decoding_failed")) }
    return text
  }
