...
public struct FKSecurityCoder: FKSecurityCoding, Sendable {
  ...
  public func base64Decode(_ string: String) throws -> Data {
    guard let data = Data(base64Encoded: string) else {
      throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.detail.invalid_base64"))
    }
    return data
  }
  ...
  public func data(fromHex hex: String) throws -> Data {
    ...
    guard cleaned.count % 2 == 0 else {
      throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.detail.hex_length_even"))
    }
    ...
    while index < cleaned.endIndex {
      ...
      guard let byte = UInt8(byteString, radix: 16) else {
        throw FKSecurityError.invalidInput(FKI18n.format("fkcore.security.detail.hex_invalid_byte", byteString))
      }
      data.append(byte)
      index = next
    }
