...
enum FKASN1 {
  ...
  struct Reader {
    ...
    mutating func readElement() throws -> (tag: UInt8, content: Data) {
      guard index < data.count else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.asn1.unexpected_end")) }
      let tag = data[index]
      index += 1
      let length = try readLength()
      guard index + length <= data.count else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.asn1.invalid_length")) }
    ...
    private mutating func readLength() throws -> Int {
      guard index < data.count else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.asn1.missing_length")) }
      ...
      guard byteCount > 0, byteCount <= 4 else {
        throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.asn1.unsupported_length"))
      }
      guard index + byteCount <= data.count else {
        throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.asn1.invalid_length_bytes"))
      }
      var value = 0
  ...
  static func objectIdentifier(_ oid: [UInt64]) throws -> Data {
    guard oid.count >= 2 else {
      throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.asn1.invalid_oid"))
    }
    var body = Data()
