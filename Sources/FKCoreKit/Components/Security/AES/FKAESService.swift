...
public final class FKAESService: FKAESCrypting, @unchecked Sendable {
  ...
  public func generateKey(length: Int = kCCKeySizeAES256) async throws -> Data {
    try await executor.run {
      guard [kCCKeySizeAES128, kCCKeySizeAES192, kCCKeySizeAES256].contains(length) else {
        throw FKSecurityError.invalidKey(FKI18n.string("fkcore.security.detail.aes_key_length"))
      }
      ...
      guard status == errSecSuccess else {
        throw FKSecurityError.securityFailed(status: status, message: FKI18n.string("fkcore.security.detail.sec_random_failed"))
      }
      return data
    }
  }
  ...
  public func decryptString(_ base64Ciphertext: String, using key: Data, iv: Data?, mode: FKAESMode) async throws -> String {
    guard let data = Data(base64Encoded: base64Ciphertext) else {
      throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.detail.invalid_ciphertext_base64"))
    }
    let decrypted = try await decrypt(data, using: key, iv: iv, mode: mode)
    guard let string = String(data: decrypted, encoding: .utf8) else {
      throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.detail.utf8_decode_failed"))
    }
    return string
  }
  ...
  private func validate(key: Data, iv: Data?, mode: FKAESMode) throws {
    ...
    case .cbc:
      guard let iv, iv.count == kCCBlockSizeAES128 else {
        throw FKSecurityError.invalidKey(FKI18n.string("fkcore.security.detail.aes_iv_length"))
      }
    case .ecb:
      break
    }
  ...
  private func crypt(
    ...
    try await executor.run {
      ...
      guard status == kCCSuccess else {
        throw FKSecurityError.cryptoFailed(status: status, message: FKI18n.string("fkcore.security.detail.cccrypt_failed"))
      }
  ...
  private func cryptFile(
    ...
    try await executor.run {
      ...
      do {
        ...
        throw FKSecurityError.fileFailed(FKI18n.format("fkcore.security.detail.cannot_open_input_file", inputURL.path))
      }
      ...
      do {
        ...
        throw FKSecurityError.fileFailed(FKI18n.format("fkcore.security.detail.cannot_open_output_file", outputURL.path))
      }
      ...
      guard createStatus == kCCSuccess, let cryptor else {
        throw FKSecurityError.cryptoFailed(status: createStatus, message: FKI18n.string("fkcore.security.detail.cccrypt_failed"))
      }
      ...
      guard finalStatus == kCCSuccess else {
        throw FKSecurityError.cryptoFailed(status: finalStatus, message: FKI18n.string("fkcore.security.detail.cccrypt_failed"))
      }
