  ...
  public func generateKeyPair(keySize: Int = 2048, tag: String, storeInKeychain: Bool = true) async throws -> FKRSAKeyPair {
    try await executor.run {
      guard [2048, 3072, 4096].contains(keySize) else {
        throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.detail.rsa_key_size"))
      }
      ...
      guard let privateKey = SecKeyCreateRandomKey(parameters as CFDictionary, &error) else {
        let message = (error?.takeRetainedValue() as Error?)?.localizedDescription ?? FKI18n.string("fkcore.security.detail.sec_key_create_random_failed")
        throw FKSecurityError.securityFailed(status: errSecParam, message: message)
      }
      guard let publicKey = SecKeyCopyPublicKey(privateKey) else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.sec_key_public_nil"))
      }
  ...
  public func encrypt(_ data: Data, publicKey: SecKey, algorithm: FKRSAEncryptionAlgorithm) async throws -> Data {
    try await executor.run {
      ...
      guard SecKeyIsAlgorithmSupported(publicKey, .encrypt, secAlg) else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.rsa_encrypt_unsupported"))
      }
      guard let out = SecKeyCreateEncryptedData(publicKey, secAlg, data as CFData, &error) as Data? else {
        let message = (error?.takeRetainedValue() as Error?)?.localizedDescription ?? FKI18n.string("fkcore.security.detail.sec_key_encrypt_failed")
  ...
  public func decrypt(_ data: Data, privateKey: SecKey, algorithm: FKRSAEncryptionAlgorithm) async throws -> Data {
    try await executor.run {
      ...
      guard SecKeyIsAlgorithmSupported(privateKey, .decrypt, secAlg) else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.rsa_decrypt_unsupported"))
      }
      guard let out = SecKeyCreateDecryptedData(privateKey, secAlg, data as CFData, &error) as Data? else {
        let message = (error?.takeRetainedValue() as Error?)?.localizedDescription ?? FKI18n.string("fkcore.security.detail.sec_key_decrypt_failed")
        throw FKSecurityError.unknown(message)
      }
      return out
    }
  ...
  public func sign(_ data: Data, privateKey: SecKey, algorithm: FKRSAKindOfSignature) async throws -> Data {
    try await executor.run {
      ...
      guard SecKeyIsAlgorithmSupported(privateKey, .sign, secAlg) else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.rsa_sign_unsupported"))
      }
      guard let out = SecKeyCreateSignature(privateKey, secAlg, data as CFData, &error) as Data? else {
        let message = (error?.takeRetainedValue() as Error?)?.localizedDescription ?? FKI18n.string("fkcore.security.detail.sec_key_sign_failed")
        throw FKSecurityError.unknown(message)
      }
      return out
    }
  ...
  public func verify(_ signature: Data, data: Data, publicKey: SecKey, algorithm: FKRSAKindOfSignature) async throws -> Bool {
    try await executor.run {
      ...
      guard SecKeyIsAlgorithmSupported(publicKey, .verify, secAlg) else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.rsa_verify_unsupported"))
      }
  ...
  public func exportPublicKeySPKIDER(_ publicKey: SecKey) async throws -> Data {
    try await executor.run {
      guard let raw = SecKeyCopyExternalRepresentation(publicKey, nil) as Data? else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.sec_key_export_public_nil"))
      }
  ...
  public func exportPrivateKeyPKCS8DER(_ privateKey: SecKey) async throws -> Data {
    try await executor.run {
      guard let raw = SecKeyCopyExternalRepresentation(privateKey, nil) as Data? else {
        throw FKSecurityError.unavailable(FKI18n.string("fkcore.security.detail.sec_key_export_private_nil"))
      }
...
extension FKRSAService {
  private static func unwrapPKCS8(_ der: Data) throws -> Data {
    ...
    guard tag == 0x30 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.pkcs8.expected_sequence")) }
    ...
    guard pkTag == 0x04 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.pkcs8.expected_octet_string")) }
    return pkContent
  }
  ...
  private static func unwrapSPKI(_ der: Data) throws -> Data {
    var reader = FKASN1.Reader(der)
    let (tag, content) = try reader.readElement()
    guard tag == 0x30 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.spki.expected_sequence")) }

    var seq = FKASN1.Reader(content)
    _ = try seq.readElement() // algorithm SEQUENCE
    let (bsTag, bsContent) = try seq.readElement()
    guard bsTag == 0x03 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.spki.expected_bit_string")) }
    guard !bsContent.isEmpty else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.spki.empty_bit_string")) }
  ...
  private static func createRSAPublicKey(fromPKCS1 data: Data) throws -> SecKey {
    ...
    guard let key = SecKeyCreateWithData(data as CFData, attributes as CFDictionary, &error) else {
      let message = (error?.takeRetainedValue() as Error?)?.localizedDescription ?? FKI18n.string("fkcore.security.detail.sec_key_create_public_failed")
      throw FKSecurityError.unknown(message)
    }
    return key
  }
  ...
  private static func createRSAPrivateKey(fromPKCS1 data: Data) throws -> SecKey {
    ...
    var error: Unmanaged<CFError>?
    guard let key = SecKeyCreateWithData(data as CFData, attributes as CFDictionary, &error) else {
      let message = (error?.takeRetainedValue() as Error?)?.localizedDescription ?? FKI18n.string("fkcore.security.detail.sec_key_create_private_failed")
      throw FKSecurityError.unknown(message)
    }
    return key
  }
...
extension FKRSAService {
  ...
  private static func rsaModulusBitLength(fromPKCS1PublicKey der: Data) throws -> Int {
    var reader = FKASN1.Reader(der)
    let (tag, content) = try reader.readElement()
    guard tag == 0x30 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.rsa_public.expected_sequence")) }
    var seq = FKASN1.Reader(content)
    let (modTag, modContent) = try seq.readElement()
    guard modTag == 0x02 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.rsa_public.expected_modulus")) }
  ...
  private static func rsaModulusBitLength(fromPKCS1PrivateKey der: Data) throws -> Int {
    var reader = FKASN1.Reader(der)
    let (tag, content) = try reader.readElement()
    guard tag == 0x30 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.rsa_private.expected_sequence")) }
    var seq = FKASN1.Reader(content)
    _ = try seq.readElement() // version INTEGER
    let (modTag, modContent) = try seq.readElement()
    guard modTag == 0x02 else { throw FKSecurityError.invalidInput(FKI18n.string("fkcore.security.parse.rsa_private.expected_modulus")) }
    let modulus = stripLeadingZero(modContent)
    return modulus.count * 8
  }
