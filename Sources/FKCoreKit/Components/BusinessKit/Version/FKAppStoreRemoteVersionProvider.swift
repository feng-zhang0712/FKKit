...
public final class FKAppStoreRemoteVersionProvider: FKRemoteVersionProviding, @unchecked Sendable {
  ...
  public func fetchRemoteVersion() async throws -> FKRemoteVersionInfo {
    ...
    guard var components = URLComponents(string: "https://itunes.apple.com/lookup") else {
      throw FKBusinessError.invalidArgument(FKI18n.string("fkcore.business.version.lookup.invalid_base_url"))
    }
    ...
    guard let url = components.url else {
      throw FKBusinessError.invalidArgument(FKI18n.string("fkcore.business.version.lookup.build_url_failed"))
    }

    // Validate HTTP response before decoding payload.
    let (data, response) = try await session.data(from: url)
    guard (response as? HTTPURLResponse)?.statusCode == 200 else {
      throw FKBusinessError.networkFailed(underlying: FKI18n.string("fkcore.business.version.lookup.non_200_response"))
    }
    ...
    guard let first = decoded.results.first, let version = first.version else {
      throw FKBusinessError.networkFailed(underlying: FKI18n.string("fkcore.business.version.lookup.no_version_found"))
    }
