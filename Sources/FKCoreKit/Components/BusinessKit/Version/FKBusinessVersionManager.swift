...
public final class FKBusinessVersionManager: FKBusinessVersioning, @unchecked Sendable {
  ...
  public func checkForUpdate(
    ...
    if #available(iOS 13.0, *) {
      ...
      completion(.failure(.unsupported(FKI18n.string("fkcore.business.version.requires_ios_13"))))
    }
  }
  ...
  private static func buildMessage(remote: FKRemoteVersionInfo) -> String {
    var parts: [String] = []
    parts.append(FKI18n.format("fkcore.business.version.update_available.latest_version", remote.version))
    if let notes = remote.releaseNotes, !notes.isEmpty {
      parts.append(notes)
    }
    if remote.isForceUpdate {
      parts.append(FKI18n.string("fkcore.business.version.update_available.force_message"))
    }
