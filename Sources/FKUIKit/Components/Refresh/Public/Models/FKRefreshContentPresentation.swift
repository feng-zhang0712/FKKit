...
public enum FKRefreshStatusTextMode: Sendable {
  ...
  case full
  /// Indicator only — no status label. VoiceOver still uses ``FKRefreshText`` when available.
  case indicatorOnly
}

/// Footer behavior after ``FKRefreshState/noMoreData`` is reached.
public enum FKRefreshNoMoreDataBehavior: Sendable {
  /// Keep the footer visible (shows ``FKRefreshText/footerNoMoreData`` when ``FKRefreshStatusTextMode/full``).
  case showFooter
