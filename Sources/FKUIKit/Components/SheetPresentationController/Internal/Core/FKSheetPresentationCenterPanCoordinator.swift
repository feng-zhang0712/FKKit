...
final class FKSheetPresentationCenterPanCoordinator {
  ...
  func handlePan(
    ...
    let progress = FKSheetPresentationInteractionSupport.centerDismissProgress(
      translationY: downwardTranslation,
      containerHeight: actions.containerHeight()
    )
    ...
    case .changed:
      ...
      // Once scroll has consumed the pan, stay sticky — do not re-enter dismiss mid-gesture.
    ...
    case .ended, .cancelled, .failed:
      ...
      if stickyDeferredToScrollView || scrollConsumedPanDuringGesture {
        // Ensure visuals are clean if a late scroll handoff skipped an earlier reset.
        actions.resetInteractiveDismiss(false)
        actions.notifyProgress(0)
      ...
      let shouldDismiss = progress > actions.dismissProgressThreshold()
        || (downwardTranslation > 8 && velocityY > actions.dismissVelocityThreshold())
      if shouldDismiss {
