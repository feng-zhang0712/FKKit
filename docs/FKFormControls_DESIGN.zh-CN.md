...
|------|----------------------|------|
| 数值/区间 | **`BinaryFloatingPoint`/`Comparable` Extension** | 自写 clamp |
| 无障碍/本地化 | **`FKI18n`** | 硬编码 |
| 动画/Reduce Motion | **`UIView` Animation Extension**、`UIAccessibility` | 忽略 AX |
| 触觉 | 与 **`FKButton`** 一致 | 散落 generator |
| 布局 | **`UIStackView`/`CGRect` Extension** | 重复 layout 辅助 |
