...
| Search scope 条 | [FKSearchBar 设计](FKSearchBar-FKSearchField_DESIGN.zh-CN.md) 第二阶段 | 依赖 `FKSegmentedControl` |
...
└────────────────────────────┬────────────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────────────┐
│ 下游：ListKit preset · FKAlert 门控 · SearchBar scope · CellKit │
└─────────────────────────────────────────────────────────────────┘
```
...
> **目录结构说明（非强制）：** 须保持可发现性，在组件 `README.md` 中文档化。详见 [COMPONENT_ROADMAP — 组件源码目录规范](COMPONENT_ROADMAP.zh-CN.md#组件源码目录规范)。
...
FKFormControls **禁止**单次 PR 实现五个控件。与 [COMPONENT_ROADMAP 分阶段 C/D](COMPONENT_ROADMAP.zh-CN.md#分阶段发布计划) 对齐：
...
| **FKAlert** 危险勾选 | `UISwitch` | v1.1 可评估 `FKCheckbox`（[FKAlert 设计](FKAlert_DESIGN.zh-CN.md) §23） |
| **FKSearchBar** scope 条 | 未实现 | 第二阶段嵌入 `FKSegmentedControl` |
| **FKCellKit** 交互 Cell | 规划嵌入 FormControls | Phase 6 映射，见 [FKCellKit 设计](FKCellKit_DESIGN.zh-CN.md) §14.10 |
...
- [COMPONENT_ROADMAP.zh-CN.md](COMPONENT_ROADMAP.zh-CN.md) — §1.4、分阶段 C/D
- [COMPONENT_GAP_ANALYSIS.zh-CN.md](COMPONENT_GAP_ANALYSIS.zh-CN.md) — §7.2
- [FKListKit_DESIGN.zh-CN.md](FKListKit_DESIGN.zh-CN.md) — preset switch/checkbox 迁移
- [FKAlert_DESIGN.zh-CN.md](FKAlert_DESIGN.zh-CN.md) — 危险勾选门控
- [FKSearchBar-FKSearchField_DESIGN.zh-CN.md](FKSearchBar-FKSearchField_DESIGN.zh-CN.md) — scope 条依赖
- [FKCellKit_DESIGN.zh-CN.md](FKCellKit_DESIGN.zh-CN.md) — 交互 Cell 嵌入
