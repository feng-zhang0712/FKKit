...
**状态：** 草案  
**路线图引用：** [COMPONENT_ROADMAP.md](COMPONENT_ROADMAP.md) §2.7  
...
### 4.1 FKCoreKit 复用要求（强制）

| 能力 | 必须使用（FKCoreKit） | 禁止 |
|------|----------------------|------|
| 图标着色/缩放 | **`UIImage.fk_tinted`**、**`fk_resized`** | 重复 Symbol 渲染 |
| 布局度量 | **`CGRect`/`CGSize` Extension** | 魔法数 |
| 日期/时间戳 | **`FKDateFormatting`**、`Date` Extension | 自写 formatter |
| 本地化 | **`FKI18n`** | 硬编码 |
| 动画 | **`UIView` Animation Extension**；Reduce Motion 检测 | 无限制 CA 动画 |

TabBar 指示器数学可**私有复用**，但**公开 API 不得依赖** `FKTabBar`（见 §3.2）。
