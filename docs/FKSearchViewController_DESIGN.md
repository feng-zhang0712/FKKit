...
- 尊重 `FKSearchBar.configuration.debounce.minimumQueryLength`；
- 低于阈值 → 视为空 query（不调用 remote provider）。
...
- `list.search: FKListSearchConfiguration()`
...
| Q3 | remote idle 展示 placeholder 还是空？ | 可配置 `showsResultsOnEmptyQuery`，默认 placeholder 文案 |
| Q4 | 是否内置 `FKSearchResultsProviding` Network 适配器？ | v1 **仅 Examples**；v1.1 optional helper |
| Q5 | Cancel 是否默认 pop VC？ | **否** — 仅 clear；文档展示 pop 模式 |
| Q6 | `open class` vs composition protocol？ | **`open class FKSearchViewController`** 对齐 ListKit |
