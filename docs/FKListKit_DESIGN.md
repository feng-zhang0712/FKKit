...
**路线图引用：** [COMPONENT_ROADMAP.md](COMPONENT_ROADMAP.md) §1.2  
...
详见 [COMPONENT_ROADMAP.md — 勿重复造轮子](COMPONENT_ROADMAP.md#勿重复造轮子--复用对照表)。
...
**后续（FKCellKit Phase 6）：** `FKListPresetItem` 映射到 FKCellKit Cell；ListKit 层仅保留 Item → Cell 绑定，**禁止 duplicate Auto Layout**。详见 [FKCellKit_DESIGN.md](FKCellKit_DESIGN.md) §5。
...
Example `FKListKitSearchFilterExampleViewController` 演示 `UISearchController` + 防抖筛选（**低级集成**）。`FKSearchBar` 就绪后可替换搜索 UI；**推荐**迁移至 **`FKSearchViewController`**（见 [FKSearchViewController_DESIGN.md](FKSearchViewController_DESIGN.md)）。快照 `apply` 模式不变。
...
- [FKCellKit_DESIGN.md](FKCellKit_DESIGN.md) — 预设 Cell 长期归属
- [FKImageLoader-FKImageView_DESIGN.md](FKImageLoader-FKImageView_DESIGN.md) — 图标行集成
- [FKSearchBar-FKSearchField_DESIGN.md](FKSearchBar-FKSearchField_DESIGN.md) — 搜索控件集成
