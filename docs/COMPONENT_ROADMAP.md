...
1. ~~**补齐「仅有协议、无实现」的空洞**~~ — `FKImageLoader` / `FKImageView` 已交付；其余 Pluggable 组（Feature Flag、远程配置等）仍待参考实现。
2. ~~**形成列表页技术栈**~~ — Diffable 列表基础设施（**FKListKit** v1）已与 `FKRefresh`、`FKEmptyState`、`FKSkeleton` 打通。
...
`docs/` 下的组件设计文档（例如 `FKImageLoader-FKImageView_DESIGN.md`）会给出**建议的**源码目录树——常见为 `Public/`、`Internal/`、`Extension/` 等分层。这些树用于说明一种合理的组织方式，属于**建议架构**，**不是**必须严格遵守的模板。
...
| **ListKit** | `Components/ListKit/` | 生产可用 | Diffable Table/Collection VC、预设行、Refresh/Empty/Skeleton 集成。 |
...
日常集成摩擦主要来自两类结构性空洞（**图片**与**列表**能力已于 2026 上半年交付）：

| 主题 | 缺口 | 影响 |
|------|------|------|
| ~~**图片**~~ | ✅ `FKImageLoader` + `FKImageView` 已交付 | — |
| ~~**列表**~~ | ✅ `FKListKit` v1 已交付（Diffable VC、预设行、滑动操作） | — |
| **表单与确认** | `FKTextField` 较强；周边控件（分段、开关、滑块）较弱；~~自定义居中 Alert~~ → **FKAlert v1 已交付** | 设置、筛选流仍缺 FormControls；确认流已可用 |
...
**详细设计需求：** [FKImageLoader-FKImageView_DESIGN.md](FKImageLoader-FKImageView_DESIGN.md)

---

### 1.2 FKListKit（Diffable 列表基础设施）— ✅ 已交付（v1）

**模块：** `FKUIKit/Components/ListKit/`  
**实现：** [ListKit README](../Sources/FKUIKit/Components/ListKit/README.md) · [FKKitExamples ListKit Hub](../Examples/FKKitExamples/FKKitExamples/Examples/FKUIKit/ListKit/Hub/FKListKitExamplesHubViewController.swift)

**问题（已解决）**

- ~~Pluggable Cell 协议无 Diffable VC 串联~~ → `FKDiffableTableViewController` / `FKDiffableCollectionViewController`
- ~~团队重复搭建刷新/分页/空态/骨架~~ → 与 `FKRefresh`、`FKEmptyState`、`FKSkeleton` 一等公民集成
...
| `FKDiffableTableViewController` | 持有 Table + DataSource + Refresh 的基类 VC。 |
| `FKDiffableCollectionViewController` | Compositional Layout 预设（列表、网格、内嵌列表）。 |
| `FKListPresetItem` 预设 | 纯文本、副标题、图标、开关、复选、箭头 — 与 `FKDivider` 风格一致。 |
| `FKListSwipeActionConfiguration` | 左/右滑操作封装（Table）。 |

**详细设计（活文档，与代码对齐）：** [FKListKit_DESIGN.md](FKListKit_DESIGN.md)
...
**详细设计需求：** [FKSearchBar-FKSearchField_DESIGN.md](FKSearchBar-FKSearchField_DESIGN.md)

**页面级组合（待交付）：** [FKSearchViewController_DESIGN.md](FKSearchViewController_DESIGN.md) — SearchBar + ListKit 搜索页。
...
**详细设计需求：** [FKFormControls_DESIGN.md](FKFormControls_DESIGN.md)

---

### 1.5 FKAlert（自定义居中确认框）— ✅ 已交付（v1）

**模块：** `FKUIKit/Components/Alert/`  
**实现：** [Alert README](../Sources/FKUIKit/Components/Alert/README.md) · [FKKitExamples Alert Hub](../Examples/FKKitExamples/FKKitExamples/Examples/FKUIKit/Alert/FKAlertExamplesHubViewController.swift)

**问题（已解决）**

- ~~`FKBusinessAlertManager` 仅包装系统 Alert，无法 FK 品牌化~~ → `FKAlertViewController` + `FKButton`
- ~~无队列/去重/危险操作门控的样式化路径~~ → `FKAlertCoordinator` + `presentOnce(id:)`
- ~~居中 Sheet 无内容组装~~ → 基于 `FKSheetPresentationController.centerAlert`

**已交付组件**

| 类型 | 职责 |
|------|------|
| `FKAlertContent` / `FKAlertConfiguration` | Sendable 声明式内容与策略 |
| `FKAlertViewController` | 标题、正文、图标、单行 `FKTextField`、勾选门控、按钮列 |
| `FKAlertPresenter` / `FKAlertCoordinator` | `present` / `presentOnce` / `dismiss` / `setLoading`、FIFO 队列 |
| `FKAlert` / `FKAlertPresets` | `confirm` / `prompt` 便捷 API 与场景预设 |
| `FKAlertModifier` | SwiftUI `View.fkAlert` Binding 桥接 |

**详细设计（活文档，与代码对齐）：** [FKAlert_DESIGN.md](FKAlert_DESIGN.md)
...
**详细设计需求：** [FKWebView_DESIGN.md](FKWebView_DESIGN.md)
...
**详细设计需求：** [FKBiometricAuth_DESIGN.md](FKBiometricAuth_DESIGN.md)
...
**详细设计需求：** [FKBanner-FKNoticeBar_DESIGN.md](FKBanner-FKNoticeBar_DESIGN.md)
...
**详细设计需求：** [FKChip-FKTag-FKChipGroup_DESIGN.md](FKChip-FKTag-FKChipGroup_DESIGN.md)；umbrella：[FKSmallComponents_DESIGN.md](FKSmallComponents_DESIGN.md)
...
**详细设计需求：** [FKAvatar-FKAvatarGroup-FKPresenceIndicator_DESIGN.md](FKAvatar-FKAvatarGroup-FKPresenceIndicator_DESIGN.md)；umbrella：[FKSmallComponents_DESIGN.md](FKSmallComponents_DESIGN.md)
...
**详细设计需求：** [FKCarousel-FKImageBanner_DESIGN.md](FKCarousel-FKImageBanner_DESIGN.md)
...
**详细设计需求：** [FKPhotoPicker_DESIGN.md](FKPhotoPicker_DESIGN.md)
...
**详细设计需求：** [FKFileManager_DESIGN.md](FKFileManager_DESIGN.md)（§13 ZIP）；索引 [FKFileManager_ENHANCEMENT_DESIGN.md](FKFileManager_ENHANCEMENT_DESIGN.md)
...
### 2.11 FKSearchViewController

**模块：** `FKUIKit`

**问题**

`FKSearchBar` + `FKListKit` 已交付，但「搜索框 + 结果列表」页面仍重复实现防抖、取消、loading、零结果与快照更新。
...
- 本地筛选与远程 async 搜索；in-flight 取消与 stale 丢弃。
- Search Bar 放置：navigationBar / stickyHeader / tableHeader。
- 内嵌 ListKit；空/错走 `FKEmptyState`；loading 走 SearchBar + 可选 Skeleton。
- **禁止**内嵌 `UISearchBar` 或重复 `FKDebouncer`。

**复用**

- `FKSearchBar`、`FKDiffableTableViewController`、`FKEmptyState`、`FKSkeleton`；Examples 迁移 ListKit 搜索示例。

**详细设计需求：** [FKSearchViewController_DESIGN.md](FKSearchViewController_DESIGN.md)

### 2.12 SwiftUI Bridge 扩展
...
**详细设计需求：** [FKQRCode_DESIGN.md](FKQRCode_DESIGN.md)
| **FKMarquee** | FKUIKit | 公告滚动文字；尊重「减少动态效果」。 |

**详细设计需求：** [FKMarqueeLabel_DESIGN.md](FKMarqueeLabel_DESIGN.md)
...
**详细设计需求：** [FKTheme_DESIGN.md](FKTheme_DESIGN.md)
...
| **FKBackgroundTaskManager** | FKCoreKit | `BGTaskScheduler`（App Refresh / Processing）+ `beginBackgroundTask` 短时续跑。 |
...
- 基线能力 FKKitExamples（缓存、去重、401、离线预检等）。
...
- 文档化后台传输限制与恢复流程。
- Multipart 与 Network 统一 builder；基线 Examples。
...
- Pluggable 桥接；Top VC Extension；Examples Hub。
...
2. ~~`FKListKit`~~ ✅ 已交付（`FKUIKit/ListKit`，2026-06）
3. `FKSearchBar` + 表单控件（可并行）
4. ~~`FKAlert`~~ ✅ 已交付（`FKUIKit/Alert`，2026-06）
...
| **C** | `FKListKit`（Collection）、`FKSegmentedControl`、`FKToggle` | 列表 + 基础表单控件 | ListKit ✅；FormControls ⏳ |
...
**详细设计需求：** [FKPluggable_ENHANCEMENT_DESIGN.md](FKPluggable_ENHANCEMENT_DESIGN.md)
...
3. `FKAlert` 与 `FKBusinessAlertManager` **v1 长期共存**；BusinessKit 后端开关见 [FKAlert 设计](FKAlert_DESIGN.md) §23。
...
| 2026-06-08 | `docs/component-roadmap` | 新增中文版 `COMPONENT_ROADMAP.md`。 |
| 2026-06-10 | 移除英文文档引用；小组件拆分为模块级设计文档；强化 FKCoreKit 复用强制说明 |
| 2026-06-13 | §1.2 FKListKit 标注 v1 已交付；分阶段计划与待决项同步 |
| 2026-06-13 | §1.5 FKAlert 标注 v1 已交付；活文档与分阶段计划同步 |

---

## 相关文档

- [COMPONENT_GAP_ANALYSIS.md](COMPONENT_GAP_ANALYSIS.md) — 基于当前代码库的缺口与优先级分析（中文）
...
- [FKFileManager_ENHANCEMENT_DESIGN.md](FKFileManager_ENHANCEMENT_DESIGN.md) — 文件 ZIP/后台增强索引
